...
import { ChevronLeft, Brain, Calendar, TrendingUp, AlertCircle, CheckCircle2, MoreVertical, MessageSquare, Download, Activity } from 'lucide-react';
...
const PatientDetail = () => {
    ...
    const handleExport = () => {
        ...
        report += `Risk Level: ${(data.latest_report?.risk_level || 'Normal').toUpperCase()}\n`;
        report += `Latest Cognitive Score: ${data.latest_report?.overall_score || 85}%\n\n`;
        
        report += `RECENT ACTIVITIES:\n`;
        report += `------------------\n`;
        const acts = data.activities || [];
        if (acts.length === 0) report += `No recent activities.\n`;
        acts.slice(0, 10).forEach((a: any) => {
           report += `- ${new Date(a.created_at).toLocaleDateString()}: ${a.type || 'Cognitive Activity'} - Score: ${a.score}%\n`;
        });
    ...
    const handlePrescribe = () => {
        ...
        const meds = raw ? JSON.parse(raw) : [
            { id: 1, name: 'Lisinopril', dosage: '10mg', time: '08:00', days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], taken: false },
            { id: 2, name: 'Donepezil', dosage: '5mg', time: '14:00', days: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], taken: false }
        ];
        meds.push({ ...newMed, id: Date.now(), taken: false });
        localStorage.setItem('neurovia_f2_meds', JSON.stringify(meds));
        alert('Medication immediately prescribed and synced to patient dashboard.');
    ...
    useEffect(() => {
        const fetchPatientData = async () => {
            ...
            try {
                ...
                // Merge localStorage data to reflect patient's local progress
                try {
                    const localLogs = localStorage.getItem('neurovia_f2_daily_logs');
                    const localActs = localStorage.getItem('activity_progress_f2');
                    
                    if (localActs) {
                        const acts = JSON.parse(localActs).reverse().map((a: any) => ({
                            type: a.type || 'Cognitive Activity',
                            created_at: new Date(a.date).toISOString(),
                            score: a.score,
                            metadata: { Memory: a.score, Attention: a.score }
                        }));
                        if (acts.length > 0) {
                            dash.activities = [...acts, ...(dash.activities || [])];
                        }
                    }
                    
                    if (localLogs) {
                        const logs = JSON.parse(localLogs).reverse().map((l: any) => {
                            const moodScore = { Happy: 95, Calm: 80, Neutral: 60, Anxious: 35, Sad: 20 }[l.mood as string] || 60;
                            const clarityScore = { None: 100, Low: 75, Moderate: 45, High: 15 }[l.confusion as string] || 50;
                            
                            let actScore = dash.activities?.[0]?.score || 85;
                            if (localActs) {
                                const dActs = JSON.parse(localActs).filter((a: any) => a.date === l.date);
                                if (dActs.length > 0) actScore = Math.round(dActs.reduce((s: number, a: any) => s + a.score, 0) / dActs.length);
                            }
                            
                            return {
                                created_at: new Date(l.date).toISOString(),
                                overall_score: actScore,
                                mental_clarity: clarityScore,
                                mood_score: moodScore,
                                risk_level: clarityScore < 50 ? 'moderate' : 'low'
                            };
                        });
                        if (logs.length > 0) {
                            dash.history = [...logs, ...(dash.history || [])];
                            dash.latest_report = logs[0];
                        }
                    }
                } catch(e) { console.error('Error merging local storage:', e); }
    ...
    // Format graph data from history (daily_reports)
    ...
    const riskColor = latest_report?.risk_level === 'high' ? 'text-red-600 bg-red-50 border-red-100' : 
                      latest_report?.risk_level === 'moderate' ? 'text-amber-600 bg-amber-50 border-amber-100' : 
        ...
        <div className="max-w-7xl mx-auto space-y-8 fade-in pb-20 px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className={`p-6 rounded-3xl border ${riskColor}`}>
                    ...
                    <div className="text-3xl font-black">{latest_report?.risk_level?.toUpperCase() || 'NORMAL'}</div>
                </div>
                <div className="p-6 rounded-3xl border border-(--color-border-light) bg-white">
                    ...
                    <div className="text-3xl font-black text-(--color-navy)">{latest_report?.overall_score || 85}%</div>
                </div>
                <div className="p-6 rounded-3xl border border-(--color-border-light) bg-white">
                    <div className="flex items-center gap-2 mb-2 font-bold uppercase tracking-wider text-xs text-(--color-navy)/40">
                        <Brain className="w-4 h-4" /> Latest Focus
                    </div>
                    <div className="text-xl font-bold text-(--color-navy)">Attention & Recognition</div>
                </div>
                <div className="p-6 rounded-3xl border border-(--color-border-light) bg-white">
                    <div className="flex items-center gap-2 mb-2 font-bold uppercase tracking-wider text-xs text-(--color-navy)/40">
                        <Calendar className="w-4 h-4" /> Last Check-in
                    </div>
                    <div className="text-xl font-bold text-(--color-navy)">Today at 9:30 AM</div>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-(--color-border-light) shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-sm text-(--color-navy)/40">Aggregated performance across 7 days</p>
                        </div>
                        ...
                        {activities?.slice(0, 5).map((act: any, idx: number) => (
                           <div key={idx} className="p-6 flex items-center justify-between hover:bg-(--color-surface-alt) transition-colors group">
                              ...
                              <div className="text-right">
                                 <div className="text-xl font-black text-(--color-navy)">{act.score || 0}%</div>
                                 <div className="flex items-center gap-1.5 text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full text-[10px] font-black uppercase">
