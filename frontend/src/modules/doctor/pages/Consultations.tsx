                ...
                {loading ? (
                    ...
                    filtered.map((consult) => {
                        ...
                        <Link to={`${linkBase}/consultation/${consult.id}`} key={consult.id} 
                            className={`bg-[#ede7d9] p-6 rounded-3xl border shadow-sm hover:shadow-xl hover:bg-[#f5f0e8] hover:-translate-y-[3px] transition-all duration-300 group flex items-center justify-between ${
                                consult.status === 'cancelled' 
                                    ? 'border-red-200 opacity-75 hover:opacity-100' 
                                    : 'border-[#d2c8b98c] hover:border-[#6b7c52]'
                            }`}>
