...
const MicButton = ({ listening, onClick }: { listening: boolean; onClick: () => void }) => (
  <button type="button" onClick={onClick}
    className={`w-14 h-14 rounded-full flex items-center justify-center transition-all shrink-0 ${
      listening ? 'bg-red-500 text-white animate-pulse shadow-lg shadow-red-200' : 'bg-(--color-ivory-100) border-2 border-(--color-ivory-200) text-(--color-sage) hover:bg-(--color-sage) hover:text-white'
    }`}
...
const CountdownTimer = ({ seconds, label }: { seconds: number; label: string }) => {
  ...
  return (
    <div className="mb-6">
      ...
      <div className="w-full h-2.5 bg-(--color-ivory-200) rounded-full overflow-hidden">
...
const MemoryRecall = ({ data, onComplete }: { data: any; onComplete: (correct: boolean, detail?: any) => void }) => {
  ...
  return show ? (
    <div className="text-center p-10 bg-(--color-ivory-100) rounded-2xl border border-(--color-ivory-200)">
      <CountdownTimer seconds={10} label="Memorize these words" />
      <div className="flex justify-center gap-4 flex-wrap mt-4">
        {data.words?.map((w: string, i: number) => (
          <span key={i} className="text-3xl font-black text-(--color-navy) bg-white px-6 py-3 rounded-2xl border border-(--color-ivory-200) shadow-sm">{w}</span>
        ))}
      </div>
    </div>
  ) : (
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        <input value={input} onChange={e => setInput(e.target.value)} className="flex-1 h-14 px-6 bg-(--color-ivory-100) border border-(--color-ivory-200) rounded-2xl text-(--color-navy) outline-none focus:border-(--color-sage)" placeholder="Enter words separated by commas..." />
  ...
  <div className="space-y-8 text-center">
    ...
    <div className="flex justify-center gap-4 flex-wrap">
      {data.options?.map((opt: string) => (
        <button key={opt} onClick={() => onComplete(opt === data.answer)} className="px-8 py-4 bg-(--color-ivory-100) border-2 border-(--color-ivory-200) hover:border-(--color-sage) hover:bg-(--color-sage) hover:text-white transition-all rounded-xl font-bold text-2xl">{opt}</button>
      ))}
    </div>
  </div>
);
...
const ImageRecall = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => {
  ...
  const handleSubmit = () => { const count = selected.filter(s => correctLabels.includes(s)).length; onComplete(count >= Math.ceil(correctLabels.length / 2)); };
  ...
  return show ? (
      ...
      <div className="flex justify-center gap-8 flex-wrap mt-4">
        {data.images?.map((img: any, i: number) => (
          <div key={i} className="flex flex-col items-center">
            <div className="w-24 h-24 bg-white rounded-2xl shadow border border-(--color-ivory-200) flex items-center justify-center"><span className="text-5xl">{EMOJI_MAP[img.label] || '🖼️'}</span></div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {(data.options || []).map((opt: string) => (
          <button key={opt} onClick={() => toggleOption(opt)} className={`p-4 rounded-xl border-2 font-bold text-lg transition-all flex flex-col items-center gap-2 ${selected.includes(opt) ? 'border-(--color-sage) bg-(--color-sage) text-white shadow-lg' : 'border-(--color-ivory-200) bg-(--color-ivory-100) text-(--color-navy) hover:border-(--color-sage)'}`}>
...
const DigitSpan = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => {
  ...
  useEffect(() => { setShow(true); setInput(''); const t = setTimeout(() => setShow(false), 10000); return () => clearTimeout(t); }, [data]);
  const handleSubmit = () => { onComplete(input.replace(/\s/g, '') === (data.sequence?.join('') || '')); };
  ...
  return show ? (
    <div className="text-center p-10 bg-(--color-ivory-100) rounded-2xl border border-(--color-ivory-200)">
      ...
      <div className="flex justify-center gap-4 mt-4">
        {data.sequence?.map((d: number, i: number) => (
          <span key={i} className="w-16 h-16 flex items-center justify-center text-3xl font-black text-(--color-navy) bg-white rounded-2xl border-2 border-(--color-ivory-200) shadow-sm">{d}</span>
        ))}
      </div>
    </div>
  ) : (
    <div className="space-y-8 text-center">
      ...
      <div className="flex gap-3 justify-center">
        <input value={input} onChange={e => setInput(e.target.value)} className="w-full max-w-sm h-14 px-6 bg-(--color-ivory-100) border border-(--color-ivory-200) rounded-2xl text-(--color-navy) outline-none text-center text-xl tracking-widest focus:border-(--color-sage)" placeholder="Type digits here" />
        <MicButton listening={voice.listening} onClick={voice.toggle} />
      </div>
  ...
  <div className="space-y-8 text-center">
    ...
    <div className="flex justify-center flex-wrap gap-4 max-w-md mx-auto">
      {['red', 'blue', 'green', 'yellow', 'purple'].map(c => (
        <button key={c} onClick={() => onComplete(c === data.answer)} className="px-6 py-3 min-w-[100px] rounded-xl border-2 font-bold capitalize bg-white text-(--color-navy) border-(--color-ivory-200) hover:border-(--color-sage) hover:bg-(--color-sage) hover:text-white transition-all">{c}</button>
      ))}
    </div>
  </div>
);
...
const TaskSequencing = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => {
  ...
  return show ? (
    <div className="p-10 bg-(--color-ivory-100) rounded-2xl border border-(--color-ivory-200)">
      <CountdownTimer seconds={10} label="Memorize the correct order" />
      <div className="space-y-3 mt-4">
        {data.steps?.map((step: string, idx: number) => (
          <div key={idx} className="p-4 rounded-xl border border-(--color-ivory-200) bg-white text-center text-lg font-bold text-(--color-navy)"><span className="text-(--color-sage) mr-2">{idx + 1}.</span>{step}</div>
        ))}
      </div>
    </div>
  ) : (
...
const SentenceCompletion = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => (
  <div className="space-y-6 text-center">
    ...
    <div className="grid grid-cols-2 gap-4 max-w-lg mx-auto">
      {(data.options || []).map((opt: string) => (
        <button key={opt} onClick={() => onComplete(opt === data.answer)} className="px-6 py-5 bg-(--color-ivory-100) border-2 border-(--color-ivory-200) hover:border-(--color-sage) hover:bg-(--color-sage) hover:text-white transition-all rounded-xl font-bold text-lg capitalize">{opt}</button>
      ))}
    </div>
  </div>
);
...
const SemanticFluency = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => {
  const [input, setInput] = useState('');
  const voice = useVoiceInput((text) => setInput(prev => prev ? `${prev}, ${text}` : text));
  const handleSubmit = () => { const count = input.split(',').map(w => w.trim()).filter(Boolean).length; onComplete(count >= 3); };
  return (
    <div className="space-y-6 text-center">
      ...
      <div className="flex gap-3">
        <textarea value={input} onChange={e => setInput(e.target.value)} className="flex-1 h-32 p-4 bg-(--color-ivory-100) border border-(--color-ivory-200) rounded-2xl outline-none focus:border-(--color-sage)" placeholder="Separate items with commas..." />
        <MicButton listening={voice.listening} onClick={voice.toggle} />
      </div>
...
const FamilyRecognition = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => (
  <div className="space-y-8 text-center">
    ...
    <div className="flex justify-center gap-4 pt-4 flex-wrap">
      {data.options?.map((opt: string) => (
        <button key={opt} onClick={() => onComplete(opt === data.answer)} className="px-8 py-4 bg-(--color-ivory-100) border-2 border-(--color-ivory-200) hover:border-(--color-sage) hover:bg-(--color-sage) hover:text-white transition-all rounded-xl font-bold text-lg">{opt}</button>
      ))}
    </div>
  </div>
);
...
const PhoneRecognition = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => {
  ...
  return show ? (
    ...
    <div className="space-y-6 text-center">
      ...
      <div className="flex gap-3 justify-center">
        <input value={input} onChange={e => setInput(e.target.value)} className="w-full max-w-sm h-16 px-6 bg-(--color-ivory-100) border-2 border-(--color-ivory-200) rounded-2xl outline-none focus:border-(--color-sage) text-center text-2xl font-bold tracking-widest" placeholder="___-___-____" />
        <MicButton listening={voice.listening} onClick={voice.toggle} />
      </div>
      <button onClick={() => onComplete(input.replace(/\s/g, '') === data.number)} className="w-full max-w-sm mx-auto block py-4 bg-(--color-sage) text-white rounded-xl font-bold hover:bg-[#6b8c84] transition-colors">Dial Number</button>
    </div>
  );
};
...
const WordAssociation = ({ data, onComplete }: { data: any; onComplete: (correct: boolean) => void }) => (
  <div className="space-y-8 text-center">
    ...
    <div className="grid grid-cols-2 gap-4 max-w-lg mx-auto mt-8">
      {data.options?.map((opt: string) => (
        <button key={opt} onClick={() => onComplete(opt === data.answer)} className="px-6 py-5 bg-(--color-ivory-100) border-2 border-(--color-ivory-200) hover:border-(--color-sage) hover:bg-(--color-sage) hover:text-white transition-all rounded-xl font-bold text-lg">{opt}</button>
      ))}
    </div>
  </div>
);
  ...
  return (
    <div className="text-center py-4 fade-in">
      ...
      <div className="flex justify-center gap-8 mb-8">
        ...
        <div className="bg-(--color-ivory-100) rounded-2xl p-6 min-w-[120px]"><p className="text-4xl font-black text-(--color-navy)">{pct}%</p><p className="text-sm font-bold text-(--color-navy)/50 mt-1 uppercase">Score</p></div>
      </div>
...
export const PersonalDataSetup = () => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-(--color-ivory-200) overflow-hidden">
      <button onClick={() => setShowPanel(!showPanel)} className="w-full flex items-center justify-between p-6 hover:bg-(--color-ivory-100) transition-colors">
      ...
      {showPanel && (
        <div className="p-6 pt-0 border-t border-(--color-ivory-200)">
            ...
            <div>
              ...
              <div className="space-y-3 mb-4">
                {familyMembers.map((m, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-(--color-ivory-100) rounded-xl border border-(--color-ivory-200)">
                    {m.photo ? (
              ...
              <div className="flex gap-2">
                <input value={newFamily.name} onChange={e => setNewFamily({ ...newFamily, name: e.target.value })} className="flex-1 h-10 px-3 bg-(--color-ivory-100) border border-(--color-ivory-200) rounded-lg text-sm outline-none focus:border-(--color-sage)" placeholder="Name (e.g. Sarah)" />
                <input value={newFamily.relationship} onChange={e => setNewFamily({ ...newFamily, relationship: e.target.value })} className="flex-1 h-10 px-3 bg-(--color-ivory-100) border border-(--color-ivory-200) rounded-lg text-sm outline-none focus:border-(--color-sage)" placeholder="Relationship (e.g. Daughter)" />
            ...
            <div>
              ...
              <div className="space-y-3 mb-4">
                {phoneContacts.map((c, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-(--color-ivory-100) rounded-xl border border-(--color-ivory-200)">
                    <div>
              ...
              <div className="flex gap-2">
                <input value={newPhone.name} onChange={e => setNewPhone({ ...newPhone, name: e.target.value })} className="flex-1 h-10 px-3 bg-(--color-ivory-100) border border-(--color-ivory-200) rounded-lg text-sm outline-none focus:border-(--color-sage)" placeholder="Name (e.g. Daughter)" />
                <input value={newPhone.number} onChange={e => setNewPhone({ ...newPhone, number: e.target.value })} className="flex-1 h-10 px-3 bg-(--color-ivory-100) border border-(--color-ivory-200) rounded-lg text-sm outline-none focus:border-(--color-sage)" placeholder="Number (e.g. 555-0199)" />
...
const ActivityPlayer: React.FC<ActivityPlayerProps> = ({ activityType, onComplete, onExit }) => {
  ...
  const handleNext = (correct: boolean, detail?: any) => {
    const rr: RoundResult = { round, correct, wordDetail: detail?.wordScores, correctCount: detail?.correctCount, totalWords: detail?.totalWords };
    const nr = [...results, rr]; setResults(nr);
    if (round < totalRounds) { setRound(p => p + 1); loadQuestion(); } else { finishActivity(nr); }
  };
  ...
  return (
    <div className="py-4">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-(--color-ivory-200)">
        <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          {Array.from({ length: totalRounds }).map((_, i) => (
            <div key={i} className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all ${
              ...
              : 'bg-(--color-ivory-100) text-(--color-navy)/40 border-(--color-ivory-200)'
            }`}>{i + 1}</div>
          ))}
        </div>
      </div>
